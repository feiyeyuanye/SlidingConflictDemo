...
/**
 * Created by xwxwaa on 2018/4/24.
 */
...
public class LineChartView {
     ...
     * @return LineDataTwo
     */
    public static LineData initSingleLineChart(Context context, LineChart mLineChart, int count, List<String> xValue, List<Float> yValue) {

        ArrayList<String> xValues = new ArrayList<String>();
        for (int i = 0; i < count; i++) {
            // x轴显示的数据，这里默认使用数字下标显示
            xValues.add(xValue.get(i));
        }

        // y轴的数据
        ArrayList<Entry> yValues = new ArrayList<Entry>();
        for (int i = 0; i < count; i++) {
            yValues.add(new Entry(yValue.get(i), i));
        }
        //设置折线的样式
        ...
        dataSet.setHighLightColor(Color.parseColor("#1b97d6")); // 高亮的线的颜色
    ...
    public static LineData initDoubleLineChart(Context context, LineChart mLineChart, int count, List<String> xValue,  List<Float> yValue,  List<Float> yFullValue) {
        ArrayList<String> xValues = new ArrayList<String>();
        for (int i = 0; i < count; i++) {
            // x轴显示的数据，这里默认使用数字下标显示
            xValues.add(xValue.get(i));
        ...
        ArrayList<Entry> yValues1 = new ArrayList<Entry>();
        for (int i = 0; i < count; i++) {
            yValues1.add(new Entry(yValue.get(i), i));
        }

        // y轴的数据
        ArrayList<Entry> yValues2 = new ArrayList<Entry>();
        for (int i = 0; i < count; i++) {
            yValues2.add(new Entry(yFullValue.get(i), i));
        }
    ...
    public static LineData initThirdLineChart(Context context, LineChart mLineChart, int count, List<Float> datas1,  List<Float> datas2,  List<Float> datas3){
        ArrayList<String> xValues = new ArrayList<String>();
        for (int i = 0; i < count; i++) {
            // x轴显示的数据，这里默认使用数字下标显示
            xValues.add((i+1) + "月");
        ...
        ArrayList<Entry> yValues1 = new ArrayList<Entry>();
        for (int i = 0; i < count; i++) {
            yValues1.add(new Entry(datas1.get(i), i));
        }

        // y轴的数据
        ArrayList<Entry> yValues2 = new ArrayList<Entry>();
        for (int i = 0; i < count; i++) {
            yValues2.add(new Entry(datas2.get(i), i));
        }

        //yy轴的数据
        ArrayList<Entry> yValues3 = new ArrayList<Entry>();
        for (int i = 0; i < count; i++) {
            yValues3.add(new Entry(datas3.get(i), i));
        }
        ...
        dataSet.setCircleSize(2f);// 显示的圆形大小
        dataSet.setColor(Color.rgb(95, 160, 214));// 折线显示颜色
        dataSet.setCircleColor(Color.rgb(95, 160, 214));// 圆形折点的颜色
        dataSet.setHighLightColor(Color.parseColor("#5fa0d6")); // 高亮的线的颜色
        dataSet.setHighlightEnabled(false);
        dataSet.setValueTextColor(Color.rgb(95, 160, 214)); //数值显示的颜色
        dataSet.setValueTextSize(13);     //数值显示的大小
        ...
        dataSet1.setCircleSize(2f);
        dataSet1.setColor(Color.rgb(253, 174, 103));
        dataSet1.setCircleColor(Color.rgb(253, 174, 103));
        dataSet1.setHighLightColor(Color.parseColor("#fdae67"));
        dataSet1.setHighlightEnabled(false);
        dataSet1.setValueTextColor(Color.rgb(253, 174, 103));
        dataSet1.setValueTextSize(13);

        LineDataSet dataSet3 = new LineDataSet(yValues3, lineName1);
        dataSet3.setLineWidth(1.5f);
        dataSet3.setCircleSize(2f);
        dataSet3.setColor(Color.rgb(195, 145, 190));
        dataSet3.setCircleColor(Color.rgb(195, 145, 190));
        dataSet3.setHighLightColor(Color.parseColor("#c391be"));
        dataSet3.setHighlightEnabled(false);
        dataSet3.setValueTextColor(Color.rgb(195, 145, 190));
        dataSet3.setValueTextSize(13);
